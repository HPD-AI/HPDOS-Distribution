...
$temporary = Join-Path ([System.IO.Path]::GetTempPath()) ("hpdos-install-" + :NewGuid().ToString("N"))
...
try {
    ...
    $checksumLine = Get-Content $checksums |
        Where-Object { $_ -match "(^|[ *])$(:Escape($asset))$" } |
    ...
    $staging = Join-Path $versions (".staging-$Version-" + :NewGuid().ToString("N"))
    ...
    Set-Content `
        ...
        -Value "@echo off`r`n`"$target\hpdos.exe`" %*`r`n" `
        -NoNewline
    ...
    if (Test-Path -LiteralPath $launcher) {
        ...
        if (-not $onPath) {
            ...
            :SetEnvironmentVariable(
                'Path',
                "$BinDirectory;" + :GetEnvironmentVariable('Path', 'User'),
                'User'
            )
